...
class ChildProcess:
    ...
    def _analyze(self):
        ...
        prompt += (
            ...
            f"IMPORTANT CONTEXT:\n"
            f"- The benchmark tests 3 batch sizes via the SAME API: idct_2d() for"
            f" low (10), medium (200), and high (10000) blocks.\n"
            f"- Fitness = 0.5*high + 0.3*mid + 0.2*low (lower is better).\n"
            f"- The benchmark is a black-box caller of idct_2d()."
            f" It does NOT choose CPU vs GPU — the engine must do that internally.\n"
            f"- GPU acceleration (OpenCL) wins on large batches."
            f" Implement GPU kernels via the GpuKernel trait in gpu.rs.\n"
            f"- CPU path wins on small batches (low overhead, no kernel launch cost).\n"
            f"- The winning solution: idct_2d() should auto-detect batch context"
            f" and dispatch to CPU or GPU internally.\n"
            f"- CPU parallelism (rayon, thread pools) is also viable.\n\n"
